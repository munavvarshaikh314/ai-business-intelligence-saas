...
class AnalyticsService:
    ...
    # ---------------------------
    # Helper: Load dataset rows into pandas DataFrame
    # ---------------------------
    @staticmethod
    def get_dataset_table(dataset_id: str, user_id: str) -> str:
        ...
        try:
            ...
            if not dataset.table_name:
                raise HTTPException(status_code=400, detail="Upload a CSV file to this dataset before opening analytics.")
    ...
    @staticmethod
    def load_dataset_dataframe(dataset_id: str, user_id: str):
        db: Session = SessionLocal()
        try:
            table_name = AnalyticsService.get_dataset_table(dataset_id, user_id)
            return pd.read_sql_query(text(f'SELECT * FROM "{table_name}"'), db.bind)
        finally:
            db.close()
    # ---------------------------
    ...
    def get_summary(dataset_id: str, user_id: str):
        ...
        try:
            ...
            revenue_col = AnalyticsService.first_matching_column(
                columns,
                ["revenue", "sales", "amount", "price", "total_price", "total", "profit"]
            )
            profit_col = AnalyticsService.first_matching_column(columns, ["profit", "net_profit", "margin"])
            order_col = AnalyticsService.first_matching_column(columns, ["order_id", "order", "invoice", "id"])

            total_revenue = avg_order_value = total_profit = None
            if revenue_col:
                total_revenue = db.execute(text(f'SELECT SUM("{revenue_col}") FROM "{table_name}"')).scalar()
                avg_order_value = db.execute(text(f'SELECT AVG("{revenue_col}") FROM "{table_name}"')).scalar()

            if profit_col:
                total_profit = db.execute(text(f'SELECT SUM("{profit_col}") FROM "{table_name}"')).scalar()

            total_orders = total_rows
            if order_col:
                total_orders = db.execute(text(f'SELECT COUNT(DISTINCT "{order_col}") FROM "{table_name}"')).scalar()
    ...
    def get_monthly_sales(dataset_id: str, user_id: str):
        ...
        columns = AnalyticsService.get_columns(db, table_name)
        date_col = AnalyticsService.first_matching_column(columns, ["date", "order_date", "created_at", "timestamp"])
        revenue_col = AnalyticsService.first_matching_column(columns, ["revenue", "sales", "amount", "price", "total"])
    ...
    def get_region_sales(dataset_id: str, user_id: str):
        ...
        columns = AnalyticsService.get_columns(db, table_name)
        region_col = AnalyticsService.first_matching_column(columns, ["region", "city", "state", "country", "location"])
        revenue_col = AnalyticsService.first_matching_column(columns, ["revenue", "sales", "amount", "price", "total"])
        ...
        sql = text(f"""
            SELECT "{region_col}", SUM("{revenue_col}") AS total_revenue
            FROM "{table_name}"
            GROUP BY "{region_col}"
    ...
    def get_category_sales(dataset_id: str, user_id: str):
        ...
        columns = AnalyticsService.get_columns(db, table_name)
        category_col = AnalyticsService.first_matching_column(columns, ["category", "product", "item", "type", "segment"])
        revenue_col = AnalyticsService.first_matching_column(columns, ["revenue", "sales", "amount", "price", "total"])
        ...
        sql = text(f"""
            SELECT "{category_col}", SUM("{revenue_col}") AS total_revenue
            FROM "{table_name}"
