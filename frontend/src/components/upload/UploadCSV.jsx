...
export default function UploadCSV() {
  ...
  const handleUpload = async () => {
    ...
    if (!file.name.endsWith(".csv")) {
      setError("Only .csv files are supported. Please select a valid CSV file.");
      return;
    }
    ...
    try {
      ...
      setMessage(res.message || "CSV uploaded and indexed successfully.");
      setFile(null);
    } catch (err) {
      setError(UploadErrors.csv(err));
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div>
      ...
      <p className="text-sm text-gray-600 mb-3">
        Upload a CSV file to generate an analytics dashboard and enable SQL queries.
      </p>
      ...
      <button
        ...
        disabled={loading}
        className="mt-3 rounded-md bg-slate-950 px-4 py-2 text-sm font-semibold text-white transition hover:bg-emerald-700 disabled:bg-slate-300 disabled:cursor-not-allowed"
      >
